...
import { BnumEvent } from '../js/mel_events.js';
import { MelObject } from '../js/mel_object.js';
...
export class MelPrevisualiser extends MelObject {
  ...
  generate() {
    ...
    html = this._p_show_defaut_previsu(html)
      .end('span')
      .end()
      .separate()
      .css({ 'margin-top': '10px', 'margin-bottom': '10px' });

    if (this.on_generate.haveEvents())
      html = this.on_generate.call(html, this).end();
    else html = html.end();

    if (this.on_after_html.haveEvents())
      html = this.on_after_html.call(html, this);
  ...
  create_popup(title) {
    ...
    if (this.on_after_generate_jquery.haveEvents())
      $jquery_generated_from_js_html = this.on_after_generate_jquery.call(
        $jquery_generated_from_js_html,
        this,
      );

    if (this.rcmail) {
      this.rcmail().show_popup_dialog(
        $jquery_generated_from_js_html,
        title,
        [
          {
...
export class MelListingPrevisualiser extends MelPrevisualiser {
  ...
  constructor(
    id,
    {
      previsu_id = 'bnum-previsu',
      list_container_id = 'bnum-previsu-list-container',
    },
    ...args
  ) {
  ...
  _generate_listing_elements(html) {
    html = html
      .ul({ id: this.list_container_id, class: 'folderlist ignore-bullet' })
      .css({
        'flex-wrap': 'wrap',
    ...
    for (const element of this.get_elements()) {
      already_have_li =
        element.childs.length >= 1 && element.childs[0].balise === 'li';
