...
import { NotifierObject } from '../js/NotifierObject.js';
import { VisualiserObject } from '../js/VisualiserObject.js';
import { MelEnumerable } from '../js/enum.js';
...
class RcmailDialog extends MelObject {
  ...
  show() {
    ...
    let config = {
      title: this.title,
      buttons: MelEnumerable.from(this.buttons)
        .select((x) => x.generate())
        .toArray(),
    };
    ...
    this._$dialog = $('#dialog').html($contents).dialog(config);
    return this;
  }
