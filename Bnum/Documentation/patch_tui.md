...
function buildSubNav(obj) {
	...
	var html =
		'<div class="hidden" id="' + obj.longname.replace(/"/g, '_') + '_sub">';
	...
	if (items.length) {
		...
		items.forEach(function (item) {
			...
			if (!hasOwnProp.call(item, 'longname')) {
				itemsNav +=
					'<li>' + linktoFn('', item.name) + buildSubNav(item) + '</li>';
	...
	if (members.globals.length) {
		...
		members.globals.forEach(function (g) {
			if (!hasOwnProp.call(seen, g.longname)) {
				...
				if (g.kind === 'typedef') {
					globalNav +=
						'<li class="hidden">' + linkto(g.longname, g.name) + '</li>';
				} else {
